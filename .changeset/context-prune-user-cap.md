...
- The latest user brief truncates-with-notice (keeps its head, on a code-point boundary so multibyte characters are never split) instead of being opaque-stubbed, so the model always reads the live instruction. The truncation applies a **whole-message budget**, not just per-block caps, so a brief split across many individually-small text blocks is still bounded in aggregate.
