...
import type { ProviderEntry } from '@atv-design/shared';
...
export function parseClaudeCodeSettings(
  ...
  if (
    ...
    warnings.push(
      `Claude Code settings.json defines apiKeyHelper ("${settings.apiKeyHelper}"). ATV Design does not execute helper scripts — please paste a key manually, or export ANTHROPIC_API_KEY in your shell before launching from terminal.`,
    );
  }
